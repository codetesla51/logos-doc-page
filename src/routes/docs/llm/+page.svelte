...
Reassignment throws a runtime error:
...
All functions accept: (url, body, header)
- GET/DELETE: url and optional header (body ignored)
- POST/PUT/PATCH: url, body, and optional header
header is a table of string key-value pairs.
body can be a table (auto-serialized to JSON, v0.4.6) or a string.
httpGet(url, header) — GET request (fetch data)
httpPost(url, body, header) — POST request (create resource)
httpPut(url, body, header) — PUT request (replace resource entirely)
httpPatch(url, body, header) — PATCH request (partial update)
httpDelete(url, header) — DELETE request (remove resource)
